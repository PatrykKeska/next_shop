...
const FullStarIcon = ({ value }: { value: number }) => {
  return (
    <svg
      key={value}
...
const HalfStarIcon = ({ value }: { value: number }) => {
  return (
    <svg
      key={value}
      xmlns='http://www.w3.org/2000/svg'
...
const GrayStarIcon = ({ value }: { value: number }) => {
  return (
    <svg
      key={value}
      xmlns='http://www.w3.org/2000/svg'
...
const renderStars = (value: number) => {
  ...
  for (let i = 0; i < fullStars; i++) {
    stars.push(<FullStarIcon value={i} />);
  }

  const decimal = value - Math.floor(value);
  if (decimal >= 0.5) {
    stars.push(<HalfStarIcon value={fullStars} />);
  }

  const emptyStars = 5 - stars.length;
  for (let i = 0; i < emptyStars; i++) {
    stars.push(<GrayStarIcon value={value} />);
  }
  return stars;
};
